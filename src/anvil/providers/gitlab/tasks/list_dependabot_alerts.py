"""List GitLab dependency-scanning vulnerabilities for the current project."""
...
def run(
    *,
    ...
    """List dependency vulnerabilities analogous to GitHub Dependabot alerts.
    ...
    __LOGGER__.info(
        f"Listed {len(alerts)} GitLab dependency alert(s) for {execution_target_name}"
    )
    actions.record(
        f"Listed {len(alerts)} GitLab dependency alert(s) for {execution_target_id} region {region}"
    )
    return {
