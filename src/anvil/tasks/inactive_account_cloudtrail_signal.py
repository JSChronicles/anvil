...
class CloudTrailOptions:
    """Validated metadata options for inactive account CloudTrail reporting."""
...
def load_options(metadata: dict[str, object]) -> CloudTrailOptions:
    """Load and validate CloudTrail signal metadata."""
    return CloudTrailOptions(
        athena=get_athena_options(metadata),
...
def require_string(source: dict[object, object], key: str, metadata_path: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise RuntimeError(
            f"inactive_account_cloudtrail_signal requires {metadata_path}"
        )
    return value.strip()
...
def get_cloudtrail_activity_signal(
    session, account_id: str, options: CloudTrailOptions
) -> dict[str, object]:
...
def run(
    *,
    ...
    """Collect only the inactive-account CloudTrail/Athena activity signal."""
    region_name = session.region_name
    options = load_options(metadata)
    signal = get_cloudtrail_activity_signal(session, account_id, options)
    warnings = signal.pop("warnings", [])

    actions.record(f"Collected inactive-account CloudTrail signal for {account_id}")
    __LOGGER__.info(
        f"Collected inactive-account CloudTrail signal for "
        f"{account_alias} ({account_id}), region={region_name}, dry_run={dry_run}"
    )

    result: dict[str, object] = {
        "record_type": "inactive_account_cloudtrail_signal",
